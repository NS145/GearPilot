    ...
    <Layout title="Employees">
      ...
      <Modal isOpen={modal} onClose={() => setModal(false)} title={editing ? 'Edit Employee' : 'Add Employee'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Password {editing ? '' : '*'}</label>
            <input
              className="input border-red-200 focus:ring-red-500 font-mono"
              placeholder={editing ? "Change login password..." : "Set login password..."}
              type="text"
              value={form.plainPassword}
              onChange={e => setForm(p => ({ ...p, plainPassword: e.target.value }))}
              required={!editing}
            />
            <div className="text-xs text-gray-500 mt-1">
              {editing ? "Editing this will securely change the employee's login credentials." : "This will be the employee's initial password."}
            </div>
          </div>
          <div>
